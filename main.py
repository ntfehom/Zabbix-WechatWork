...
from Functions.EventUpdate import *
from Functions.HandleUserSendTxt import *
...
def basic_get():
    ...
    if callback==-1:
        logger.error('回调解析失败，尝试调用HandleCallback时返回-1，访问者IP：'+IP)
        return ''
    return (callback)
...
def basic_post():
    ...
    try:
        ...
        if ReceiveMsg == -1:
            logger.error('解析用户发送数据失败，尝试调用DecodeMessage时返回-1，访问者IP：' + IP)
            return ''
